...
//	try
	  ...
	  while (!TableMainMinerals->Eof) {
		if (TableMainMinerals->FieldByName(fldELEMENT)->AsString ==
		  ...
		  S = TableMainMinerals->FieldByName(DBListBoxMainMinerals->DataField)->AsString;
...
void __fastcall TfmMain::SetLanguage() {
  ...
  if (Language != LANG_ENGLISH) {textdomain("Geology"); }
   ...
   case LANG_ENGLISH:
	UseLanguage("en");
	  ...
	  TableElements->TableName =
		UpperCase(AppPath+"Help\\en\\Reference\\Property\\Elements");
	  TableMainMinerals->TableName =
		UpperCase(AppPath+"Help\\en\\Reference\\Property\\MainMinerals");
   break;

   case LANG_RUSSIAN:
	  UseLanguage("ru");
	  TableElements->TableName =
		UpperCase(AppPath+"Help\\ru\\Reference\\Property\\Elements");
	  TableMainMinerals->TableName =
		UpperCase(AppPath+"Help\\ru\\Reference\\Property\\MainMinerals");
   break;
...
{
 if (MessageDlg(ButtonPrint->Caption+"?", mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0) == mrYes)
	Print();
}
//---------------------------------------------------------------------------
