...
type
   ...
   public
      property Items[Index: Integer]: TGLFloat read GetItems write SetItems; default;
...
type
   ...
   public
      property Coordinates[_Index: Integer]: TGLFloat read GetCoordinates write SetCoordinates;
         default;
...
type
   ...
   protected
      constructor Create;
      property OnChange: TNotifyEvent read FOnChange write FonChange;
      property Attenuations[Index: Integer]: TGLFloat read GetAttenuations write SetAttenuations;
   public
      destructor Destroy; override;
   ...
   published
      ...
      property ConstantAttenuation: TGLFloat index 0 read GetAttenuations write SetAttenuations;
      property LinearAttenuation: TGLFloat index 1 read GetAttenuations write SetAttenuations;
      property QuadraticAttenuation: TGLFloat index 2 read GetAttenuations write SetAttenuations;
      ...
      property SpotDirection: TGLVector3Df read FSpotDirection write FSpotDirection;
...
type
   ...
   public
      ...
      procedure SolidSphere(X, Y, Z, Radius: TGLDouble; N1: Integer = 5; N2: Integer = 5);
      procedure TextOut(Text: AnsiString;
         x1, y1, z1, Ax, Ay, Az, ScaleX, ScaleY, ScaleZ: TGLDouble; aColor: TColor);
   ...
   public
      ...
      property SelectPrimitivesArray: TArrayOfDrawPrimitive read FSelectPrimitivesArray;
      property AppendPrimitives: Boolean read FAppendPrimitives write SetAppendPrimitives;
   ...
   public
      property Triangulator: PGLUTriangulatorObj read GetTriangulator write SetTriangulator;
      property RenderMode: TRenderMode read FRenderMode write FRenderMode default rmRENDER;
      ...
      property WorldCursorX: TGLDouble index 0 read GetWorldCursor write SetWorldCursor;
      property WorldCursorY: TGLDouble index 1 read GetWorldCursor write SetWorldCursor;
      property WorldCursorZ: TGLDouble index 2 read GetWorldCursor write SetWorldCursor;
      property WorldCursorW: TGLDouble index 3 read GetWorldCursor write SetWorldCursor;
   ...
   published
      ...
      property InitialRotation: TInitialRotation read FInitialRotation write FInitialRotation;
      property RotationSequence: TRotationSequence read FRotationSequence write FRotationSequence;
      ...
      property OnChange: TNotifyEvent read FOnChange write FOnChange;
      property OnSelectPrimitive: TSelectPrimitiveEvent
         read FOnSelectPrimitive write FOnSelectPrimitive;
      property OnSelectPrimitives: TSelectPrimitivesEvent
         read FOnSelectPrimitives write FOnSelectPrimitives;
      property OnRejectPrimitive: TRejectPrimitiveEvent
         read FOnRejectPrimitive write FOnRejectPrimitive;
   public
      procedure ViewScale;
      procedure ViewAxis;
      procedure ViewTop;
...
procedure Cross(ax, ay, az, bx, by, bz, cx, cy, cz: Double; var nx, ny, nz: Double);
...
procedure TGBCanvas.DrawCube(CentX, CentY, CentZ, SizeX, SizeY, SizeZ: TGLFloat);
var
   I: Integer;
   Nx, Ny, Nz: Double;
   GL_Mode: Cardinal;
...
begin
   if (FItems[0] <> R) or (FItems[1] <> G) or (FItems[2] <> B) or (FItems[3] <> A) then
   begin
      InitRGBAf(R, G, B, A);
      Change;
   end;
...
begin
   ...
   begin
      ...
      glLightfv(GL_LIGHT0 + Number, GL_SPOT_DIRECTION, @(SpotDirection.FCoordinates));
   end
   else
   begin
...
var
   ...
   begin
      ...
      begin (*  for each hit  *)
         NameCount := ptr[K];
         Primitive.Z := (ptr[Inc1(K)] / High(TGLUint) + ptr[Inc1(K)] / High(TGLUint)) / 2;
...
begin
   ...
   begin
      ...
      begin
         ...
         glVertex3f(0, AxisRadius * Sin(Theta + Delta), AxisRadius * Cos(Theta + Delta));
         glVertex3f(AxisLength, AxisRadius * Sin(Theta + Delta), AxisRadius *
            Cos(Theta + Delta));
         glend;
      end;
      ...
      begin
         ...
         glVertex3f(AxisRadius * Sin(Theta + Delta), 0, AxisRadius * Cos(Theta + Delta));
         glVertex3f(AxisRadius * Sin(Theta + Delta), -AxisLength, AxisRadius *
            Cos(Theta + Delta));
         glend;
      end;
...
procedure TGBCanvas.DefaultKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
const
   DAX = 1;
   DAY = 1;
   DAZ = 1;
...
procedure TGBCanvas.DefaultMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
const
...
begin
   ...
   begin
      try
         Cross(Points[2].X, Points[2].Y, Points[2].Z,
...
begin
   ...
   case RenderMode of
      rmSELECT:
      begin
         gluPickMatrix(CursorX, Height - CursorY, PickWidth, PickHeight,
            viewport)
      end;
   end;
   ...
   case ProjectionMode of
      ...
      begin
         ...
         glOrtho(CenterX - Radius, CenterX + Radius,
...
procedure TGBCanvas.SetWorldCursor(const Index: Integer; const Value: TGLDouble);
begin
...
begin
   SetProjection;
   gluProject(WorldCursorX, WorldCursorY, WorldCursorZ,
      ViewMatrix, ProjectMatrix, ViewPort, @FCursor.X, @FCursor.Y, @FCursor.Z);
...
procedure TGBCanvas.SetBounds3D(AMinX, AMaxX, AMinY, AMaxY, AMinZ, AMaxZ: TGLDouble);
begin
   MinX := AMinX;
   MaxX := AMaxX;
   MinY := AMinY;
...
procedure TGBCanvas.SolidSphere(X, Y, Z, Radius: TGLDouble; N1: Integer = 5; // lines of longitude
...
procedure TGBCanvas.TextOut(Text: AnsiString;
   x1, y1, z1, Ax, Ay, Az, ScaleX, ScaleY, ScaleZ: TGLDouble; aColor: TColor);
begin
   SetPenColor(aColor);
...
procedure TGBCanvas.TextOut3D(const Text: string; X, Y, Z: Single; Color: TColor);
...
begin
   ...
   try
      if glIsList(FontStartGLList) = FALSE then
      begin
      ...
      begin
         ...
         wglUseFontOutlines(DC, 0, 255, FontStartGLList,
            0, 0.1, WGL_FONT_POLYGONS, @agmf);
      end;
   finally
...
function TGBCanvas.RejectPrimitive(x, y: Integer; Primitive: TDrawPrimitive): Boolean;
begin
   Result := False;
...
begin
   ...
   gluUnProject(Round(X), RealY, Zval,
      mvMatrix, ProjMatrix, Viewport, @FWorldCursor.X, @FWorldCursor.Y, @FWorldCursor.Z);
end;
   ...
   begin
      ...
      try
         ...
         try
            ...
            if not wglUseFontOutlines(DC, Ord(C), 1, FFontLists[C], 0,
               0.1, WGL_FONT_POLYGONS, @agmf) then
            begin
